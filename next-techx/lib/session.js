export const PullOutOfSession = async () => {
  ...
  const CheckSignature = await fetch(
    "https://techx-server.tech:443/session/CheckToken", // <<----- Проверяем подпись токена,
    {
  ...
  if (success) {
    ...
    const PullData = await fetch("https://techx-server.tech:443/session/PullOutOfSession", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("token")}`,
