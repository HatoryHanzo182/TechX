...
{                   // Handler for adding data to local storage
                   // ⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔
  ...
  try { current_data = JSON.parse(fs.readFileSync(file_path)); } 
  catch (error) { return { success: false, message: "Error reading file" }; }
  
  current_data.push(data);
  
  try { fs.writeFileSync(file_path, JSON.stringify(current_data, null, 2)); } 
...
{                     // It creates a folder for the product images, copies the images to that folder, updates the corresponding
  ...
  try { json_data = JSON.parse(fs.readFileSync(json_file_path)); } 
  catch (error) { console.error("Error reading JSON file:", error); }
  
  if (json_data.length > 0)
    json_data[json_data.length - 1].images = new_img_paths;
  
  try { fs.writeFileSync(json_file_path, JSON.stringify(json_data, null, 2)); } 
...
{                       // It first reads a JSON file containing product information from the specified path,
  ...
  try { fs.writeFileSync(file_path, JSON.stringify(new_data, null, 2)); } 
  catch (error) { return { success: false, message: "Error writing to file" }; }
  
  for (const img_path of image_paths) 
  {
    try { fs.unlinkSync(img_path); } 
...
{                                               // - Reads the current data from the JSON file.
  ...
  try 
  {
    ...
    const product_index = products.findIndex((product) => product.brand === new_data.Brand && product.category === new_data.Category && product.model === new_data.Model);
...
{
  const filePath = path.join(__dirname,"src", "CompassLocalStorage", "hashed_password.json");
  ...
  {
    return { success: false, message: `Error writing to file: ${error.message}` };
  }
});
...
ipcMain.handle("GetPasswordHashFromLocalStorage", (event) => 
{
  ...
  try 
  {
    ...
    return { success: true, message: "Data read successfully", hash: data.hash };
  } 
  catch (error) 
  {
    return { success: false, message: "Error loading the hashed password: " + error.message };
  }
});
//#endregion
