...
function CreateMainWindow() {
  ...
  _app_window.loadFile(
    path.join(__dirname, "src", "components", "AdminMenu.html"),
  );
...
app.whenReady().then(() =>
  // An event handler that executes when
  {
    // the application is ready for use.
    CreateMainWindow();
    CreateTray();

    ipcMain.on("ShowAdminPanel", () => {
      _app_window.loadFile(
        path.join(__dirname, "src", "components", "AdminMenu.html"),
      );
    });
    ipcMain.on("ShowAuthorization", () => {
      _app_window.loadFile(
        path.join(__dirname, "src", "components", "Authorization.html"),
      );
    });
...
ipcMain.handle(
  ...
  (
    event,
    data, // Handler for adding data to local storage
  ) => {
    // ⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔⛔
    ...
    try {
      ...
      return { success: false, message: "Error writing to file" };
    }
...
ipcMain.handle(
  "AddNewProductImgToLocalStorage",
  async (
    event,
    data, // Add new product images to local storage.
  ) => {
    ...
    const product_img_folder_path = path.join(
      __dirname,
      "src",
      "CompassLocalStorage",
      "ProductsImg",
...
ipcMain.handle(
  ...
  (
    event, // Processes a request to retrieve products from local storage.
  ) => {
...
ipcMain.handle(
  "UpdateProductStatusInLocalStorage",
  async (
    event,
    new_status, // Reads a JSON file containing product information
  ) => {
    ...
    try {
      ...
      const product_index = products.findIndex(
        (product) => product.model === new_status.model,
      );
...
ipcMain.handle(
  ...
  (
    event,
    data, // Processes a request to remove a product from local storage.
  ) => {
    ...
    const product = current_data.find(
    ...
    const new_data = current_data.filter(
      (product) => product.brand !== brand || product.model !== model,
    );
    ...
    for (const img_path of image_paths) {
      try {
        ...
        console.error(`Error deleting image ${img_path}:`, error);
      }
    }
...
ipcMain.handle(
  "UpdateCangeProductData",
  async (
    event,
    data, // Handler for updating product data in the local storage JSON file. This method:
  ) => {
      ...
      if (product_index !== -1) {
        ...
        _deleted_img.forEach((image_path) => {
          ...
          if (image_index !== -1) {
            ...
            try {
              ...
              console.error(`Error deleting image file: ${image_path}`, error);
            }
          }
        });
        ...
        for (const img_path of _img_product_paths_change_data7) {
          ...
          try {
            ...
            console.error("Error copying new image file:", error);
          }
        }
...
//
ipcMain.handle("UpdatePasswordHashFromLocalStorage", (event, data) => {
  const filePath = path.join(
    __dirname,
    "src",
    "CompassLocalStorage",
    "hashed_password.json",
  );

  try {
    fs.writeFileSync(filePath, JSON.stringify({ hash: data }, null, 2), "utf8");
    return { success: true, message: "Data saved successfully" };
  } catch (error) {
    return {
      success: false,
...
ipcMain.handle("GetPasswordHashFromLocalStorage", (event) => {
  const filePath = path.join(
    __dirname,
    "src",
    "CompassLocalStorage",
  ...
  try {
    ...
    return {
      success: false,
      message: "Error loading the hashed password: " + error.message,
    };
  }
});
//#endregion
